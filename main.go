...
type Training struct {
	...
	Duration 	  time.Duration 		// продолжительность тренировки **** Duration time.Duration
...
type Running struct {
	// добавьте необходимые поля в структуру *только встроить структуру трейнинг
	Training
}
...
func (w Walking) Calories() float64 {
	...
	return (((CaloriesWeightMultiplier * w.Weight + (math.Pow(w.meanSpeed())*KmHInMsec)/ w.Height * CaloriesSpeedHeightMultiplier)* CaloriesSpeedHeightMultiplier * w.Weight) * w.Duration.Hours * MinsInHour)
}
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	//(средняя_скорость_в_км/ч + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * вес_спортсмена_в_кг * время_тренировки_в_часах
	return (s.meanSpeed() + wimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * s.Duration.Hours()
}
...
func (s Swimming) TrainingInfo() InfoMessage {
	// вставьте ваш код ниже
	//ля реализации этого метода для структуры Swimming{} нужно полностью переопределить TrainingInfo() с учетом того, как рассчитывается
	//средняя скорость для плавания, дистанция и так далее. Реализация TrainingInfo() для Swimming{} будет похожа на реализацию TrainingInfo()
	//для Training{}, но с иcпользованием методов для Swimming{}
	return InfoMessage{
